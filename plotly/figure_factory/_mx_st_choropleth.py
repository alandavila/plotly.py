...
def _create_mx_states_df( state_to_st_dict):
    # URLS
    ...
    gdf['CLAVE'] = gdf['ID_1'] 
    gdf['CLAVE'] = pd.to_numeric(gdf['CLAVE'])
    gdf = gdf.rename(columns={'NAME_1': 'STATE_NAME'})

    gdf_reduced = gdf[['CLAVE', 'ID_1','STATE_NAME' ,'geometry']]

    ST = []
    for n in gdf_reduced['STATE_NAME']:
        if 'xico' in n:
            n = 'Mexico'
        if 'Michoa' in n:
            n = 'Michoacan'
...
def _calculations(df, claves, values, index, f, simplify_state, level,
                  x_centroids, y_centroids, centroid_text, x_traces,
                  y_traces, clave_polygon_map):

    if clave_polygon_map[f].type == 'Polygon':
        x = clave_polygon_map[f].simplify(
            simplify_state
        ).exterior.xy[0].tolist()
        y = clave_polygon_map[f].simplify(
            simplify_state
        ).exterior.xy[1].tolist()

        x_c, y_c = clave_polygon_map[f].centroid.xy
        state_name_str = str(df[df['CLAVE'] == f]['STATE_NAME'].iloc[0])
    ...
    elif clave_polygon_map[f].type == 'MultiPolygon':
        x = ([poly.simplify(simplify_state).exterior.xy[0].tolist() for
              poly in clave_polygon_map[f]])
        y = ([poly.simplify(simplify_state).exterior.xy[1].tolist() for
              poly in clave_polygon_map[f]])

        x_c = [poly.centroid.xy[0].tolist() for poly in clave_polygon_map[f]]
        y_c = [poly.centroid.xy[1].tolist() for poly in clave_polygon_map[f]]
        ...
        t_c = [text for poly in clave_polygon_map[f]]
...
def create_mx_st_choropleth(claves, values, scope=['mex'], binning_endpoints=None,
                      colorscale=None, order=None, simplify_state=0.02,
                      asp=None, show_hover=True,
    ...
    :param (list) claves: list of CLAVE values which correspond to the con
        catination of state and county ids. An example is '01001'.
    :param (list) values: list of numbers/strings which correspond to the
        claves list. These are the values that will determine how the counties
        are colored.
    ...
    :param (list) order: a list of the unique categories (numbers/bins) in any
        ...
        a chosen colorscale.
    :param (float) simplify_state: determines the simplification factor
    ...
    CLAVE = df_sample_r['CLAVE'].tolist()

    binning_endpoints = list(np.mgrid[min(values):max(values):4j])
    colorscale = ["#030512","#1d1d3b","#323268","#3d4b94","#3e6ab0",
                  "#4989bc","#60a7c7","#85c5d3","#b7e0e4","#eafcfd"]
    fig = ff.create_choropleth(
        CLAVE=CLAVE, values=values, scope=['Florida'], show_state_data=True,
    ...
    CLAVE = df_sample_r['CLAVE'].tolist()
    fig = ff.create_choropleth(
        CLAVE=CLAVE, values=values, scope=NE_states, show_state_data=True
    )
    ...
    fig = ff.create_choropleth(
        CLAVE=CLAVE, values=values, colorscale=colorscale,
    ...
    df_sample['State CLAVE Code'] = df_sample['State CLAVE Code'].apply(
        lambda x: str(x).zfill(2)
    )
    df_sample['County CLAVE Code'] = df_sample['County CLAVE Code'].apply(
        lambda x: str(x).zfill(3)
    )
    df_sample['CLAVE'] = (
        df_sample['State CLAVE Code'] + df_sample['County CLAVE Code']
    )
    ...
    colorscale = ["#f7fbff", "#ebf3fb", "#deebf7", "#d2e3f3", "#c6dbef",
                  ...
                  "#0b4083","#08306b"]
    CLAVE = df_sample['CLAVE']
    values = df_sample['Unemployment Rate (%)']
    fig = ff.create_choropleth(
        CLAVE=CLAVE, values=values, scope=['usa'],
    ...
    df = _create_mx_states_df(state_to_st_dict)

    clave_polygon_map = dict(
        zip(
            df['CLAVE'].tolist(),
            df['geometry'].tolist()
        )
    ...
    if len(claves) != len(values):
        raise exceptions.PlotlyError(
            'CLAVE and values must be the same length'
        )

    # make claves, values into lists
    if isinstance(claves, pd.core.series.Series):
        claves = claves.tolist()
    if isinstance(values, pd.core.series.Series):
        values = values.tolist()

    # make claves numeric
    claves = map(lambda x: int(x), claves)
    ...
    y_centroids = []
    centroid_text = []
    clave_not_in_shapefile = []
    if not binning_endpoints:
        for index, f in enumerate(claves):
            level = values[index]
            try:
                clave_polygon_map[f].type

                (x_traces, y_traces, x_centroids,
                 y_centroids, centroid_text) = _calculations(
                    df, claves, values, index, f, simplify_state, level,
                    x_centroids, y_centroids, centroid_text, x_traces,
                    y_traces, clave_polygon_map
                )
            except KeyError:
                clave_not_in_shapefile.append(f)
    ...
    if len(clave_not_in_shapefile) > 0:
        msg = (
            'Unrecognized CLAVE Values\n\nWhoops! It looks like you are '
            'trying to pass at least one CLAVE value that is not in '
            'our shapefile of CLAVE and data for the counties. Your '
            'choropleth will still show up but these counties cannot '
            'be shown.\nUnrecognized CLAVE are: {}'.format(
                clave_not_in_shapefile
            )
        )
        warnings.warn(msg)
    ...
    # aspect ratio
    if asp is None:
        usa_x_range = MX_XRANGE[1] - MX_XRANGE[0]
        usa_y_range = MX_YRANGE[1] - MX_YRANGE[0]
